...
from our_rewards import physics_accuracy_reward_evaluation, format_reward
...
def main():
    ...
    try:
        dataset_path = "chinese_dataset.json"
        test_dataset = pd.read_json(dataset_path, orient="records")
        test_dataset = test_dataset.apply(format_options, axis=1)
        ...
        logger_rewards.error(f"Failed to load dataset: {e}")
    ...
    logger_rewards.info("")
    logger_rewards.info("Evaluating using the Chinese Dataset")
    ...
    logger_rewards.info(f"Mean accuracy Reward for open-end questions: {np.mean(original_accuracy_rewards_open_end):.4f}")
    ...
    logger_rewards.info(f"Mean accuracy Reward for open-end questions: {np.mean(fine_tuned_accuracy_rewards_open_end):.4f}")
