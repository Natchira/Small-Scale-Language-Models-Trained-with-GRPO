...
def main():
    ...
    # Create two loggers
    rewards_log_file = os.path.join("output", os.path.basename(model_id) + ".rewards")
    completions_log_file = os.path.join("output", os.path.basename(model_id) + ".completions")
    ...
    # Log results
    logger_rewards.info(f"Model: {model_id}")
    logger_rewards.info("")
    ...
    ### LOGGING COMPLETIONS
    for question, answer, original_completion, original_accuracy_reward in zip(test_dataset['question'], test_dataset['answer'],original_completions, original_accuracy_rewards):
