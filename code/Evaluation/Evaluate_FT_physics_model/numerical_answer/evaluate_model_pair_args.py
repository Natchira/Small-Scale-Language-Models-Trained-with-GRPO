...
from our_rewards import (
    physics_accuracy_reward_evaluation,
    physics_accuracy_reward_extended,
    format_reward
)
import json
...
def parse_args():
    ...
    parser.add_argument("--model_id", type=str, default="meta-llama/Llama-3.2-3B-Instruct", help="Original model ID")
    parser.add_argument("--fine_tuned_model_path", type=str, default="psamtam/Llama-3.2-3B-GRPO", help="Fine-tuned model path or Hub ID")
...
def main():
    ...
    try:
        dataset = pd.read_json("combined_whole_dataset.json", orient="records")
    ...
    def accuracy_reward_details(completions, **kwargs):
        return physics_accuracy_reward_extended(
            completions,
            logger=None,
            return_details=True,   # ต้องใส่ True
            **kwargs
        )
    ...
    original_accuracy_rewards_overall.extend(original_accuracy_rewards_open_end)
    ...
    fine_tuned_accuracy_rewards_overall.extend(fine_tuned_accuracy_rewards_open_end)
    
    original_details = accuracy_reward_details(
    original_completions, answer=answers, options=options)
    fine_details     = accuracy_reward_details(
    fine_tuned_completions, answer=answers, options=options)
    
    # import json, pandas as pd, os
    os.makedirs("output", exist_ok=True)
    
    with open("output/original_details.jsonl", "w", encoding="utf-8") as f:
        for rec in original_details:
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")

    with open("output/fine_tuned_details.jsonl", "w", encoding="utf-8") as f:
        for rec in fine_details:
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")

    # (ถ้าอยาก CSV)
    pd.DataFrame(original_details).to_csv("output/original_details.csv", index=False)
    pd.DataFrame(fine_details).to_csv("output/fine_tuned_details.csv", index=False)

    logger_rewards.info("Saved detail files to output/")

    # Log results
    logger_rewards.info(f"Fine-tuned model: {fine_tuned_model_path}")
    logger_rewards.info("")
    logger_rewards.info("English Dataset:")
    ...
    n_ft = len(fine_tuned_accuracy_rewards_open_end)
    avg_accuracy_open_end_ft = tuple(sum(values) / n_ft for values in zip(*fine_tuned_accuracy_rewards_open_end))

    logger_rewards.info(
        "Mean Accuracy Reward for open-end questions (fine-tuned): "
        f"({', '.join(f'{x:.4f}' for x in avg_accuracy_open_end_ft)})"
    )
